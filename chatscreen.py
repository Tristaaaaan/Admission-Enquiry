    ...
    def remove_message(list_name, message_to_remove, message_datetime_to_remove):
        ...
        for row in result:
            if row[0] == list_name:
                ...
                Database.update_data(list_name, new_messages, new_message_datetimes, new_usernames)
                break
...
class SelectionMenu(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.height = dp(40)
        copy_button = MDIconButton(icon="content-copy", on_release=self.copy_text)
        delete_button = MDIconButton(icon="delete", on_release=self.delete_text)
    ...
    def delete_text(self, *args):
        ...
        message_datetime_to_remove = self.parent.datetime.strftime('%Y-%m-%d %H:%M:%S')

        DatabaseHandler.remove_message(list_name, message_to_remove, message_datetime_to_remove)
...
class DatabaseInterface:
    ...
    def update_chat_data(list_name, message, message_datetime, username):
        ...
        new_messages = row['message'] + "||" + message if row['message'] else message
        new_message_datetime = row['message_datetime'] + "||" + message_datetime if row['message_datetime'] else message_datetime
        new_usernames = row['username'] + "||" + username if row['username'] else username
        Database.update_data(list_name, new_messages, new_message_datetime, new_usernames)
...
class ChatScreen(Screen):
    ...
    def menu_callback(self, text_item):
        ...
        if text_item == "Interested":
            self.manager.get_screen('enquiry_list').move_to_interested(self.list_name)
        elif text_item == "Confirm":
            self.manager.get_screen('enquiry_list').move_to_confirm(self.list_name)
        elif text_item == "Rejected":
            self.manager.get_screen('enquiry_list').move_to_rejected(self.list_name)
        self.menu.dismiss()
    ...
    def _update_ui_with_data(self, row):
        ...
        message_datetimes = row['message_datetime'].split("||") if row['message_datetime'] else []
    ...
    def send_message(self, message):
        if message:
            ...
            self.ids.chat_messages.add_widget(
                MessageBubble(text=message, datetime=datetime.now(), username=username)
            )
            self.ids.text_input.text = ""
            DatabaseInterface.update_chat_data(self.list_name, message, message_datetime, username)
